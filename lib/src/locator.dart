...
class Locator {
  ...
  Future<void> hover({bool? force, double? timeout}) async {
    await frame.channel_hover(selector: selector, force: force, timeout: timeout ?? 30000.0);
  }
  ...
  Future<void> dblclick({bool? force, double? timeout}) async {
    await frame.channel_dblclick(selector: selector, force: force, timeout: timeout ?? 30000.0);
  }

  Future<String?> getAttribute(String name, {double? timeout}) async {
    final result = await frame.channel_getAttribute(selector: selector, name: name, timeout: timeout ?? 30000.0);
    return result['value'] as String?;
  }

  Future<String> inputValue({double? timeout}) async {
    final result = await frame.channel_inputValue(selector: selector, timeout: timeout ?? 30000.0);
    return result['value'] as String;
  }
  ...
  Future<bool> isEnabled({double? timeout}) async {
    final result = await frame.channel_isEnabled(selector: selector, timeout: timeout ?? 30000.0);
    return result['value'] as bool;
  }

  Future<bool> isDisabled({double? timeout}) async {
    final result = await frame.channel_isDisabled(selector: selector, timeout: timeout ?? 30000.0);
    return result['value'] as bool;
  }

  Future<bool> isChecked({double? timeout}) async {
    final result = await frame.channel_isChecked(selector: selector, timeout: timeout ?? 30000.0);
    return result['value'] as bool;
  }

  Future<bool> isEditable({double? timeout}) async {
    final result = await frame.channel_isEditable(selector: selector, timeout: timeout ?? 30000.0);
    return result['value'] as bool;
  }
  ...
  Future<void> dragTo(Locator target, {bool? force, double? timeout}) async {
    await frame.dragAndDrop(selector, target.selector, force: force, timeout: timeout);
  }
}
